...
- **自我进化**: `/maintain-signals` + `/maintain-evolve` 从 Issues、日志、版本更新三路数据驱动插件自身演化
...
```
/maintain-signals --agent all --days 30
/maintain-evolve
```
...
├── commands/                    📋 13 个用户命令 + 3 个维护者命令
│   ├── diagnose.md              运行时日志诊断
│   ├── setup-node.md            节点安装部署
│   ├── lint-config.md           配置校验
│   ├── status.md                状态总览
│   ├── qa-agent.md              QA 诊断与修复（--fix 启用修复循环）
...
│   ├── fleet-ssh.md             多 Gateway SSH
│   └── maintainer/              🔧 维护者命令（maintain- 前缀）
│       ├── maintain-signals.md  采集进化信号
│       ├── maintain-evolve.md   分析信号 + 生成进化报告
│       └── maintain-sync.md     同步知识库
├── agents/                      🤖 3 个专家 agent
...
│   └── signals.json             进化信号（/maintain-signals 输出）
├── scripts/                     辅助脚本
